...
### 6.2.6 数据选择（指标下钻、搜索与批量提交）【新增】

> 【新增内容内部说明】本小节为 2026-09-04 增量新增（依据需求理解 REQ-01 v2），只增不改，不改动本章节既有小节（6.2.1～6.2.5）与全局规则原文。与既有条目的衔接：① 可见性——【数据选择】按钮仅对管理员可见，处理方式对照全局规则 7 中【报告生成】【指标信息采集表】仅管理员显示的先例（无对应模式与提示），该约定仅在本小节内生效，全局规则 7 原文维持不变；② AI 标注——AI 返回的查询结果表格属 AI 生成内容，直接按全局规则 1 执行（"⚠ AI生成/提取，请人工核实"），不另立标注规则；③ 助手既有功能（三种模式、📎 上传、🗂 知识库引用、历史对话管理）维持现状，不因本小节调整。

功能说明：管理员可通过【数据选择】按指标层级逐层下钻（或搜索）选定指标，以**指标名 + 所处层级**确定具体数据；指标层级共 5 层——指标库 → 维度 → 主题 → 议题 → 指标；指标可多选、一次可提交多个，提交后 AI 对话框对本次提交的全部指标**一次性**返回查询结果，以**一个表格**承载。本功能下钻到指标层、勾选提交即结束，暂不选择年份/报告期等时间维度（默认值设定，后续计划扩展）。
...
2. 进入数据选择：管理员点击【数据选择】，进入数据选择界面；界面形态（弹窗/抽屉/面板）**待确认**——原型按【🗂】知识库选择的弹窗模式实现（悬浮于会话上方、不离开当前页面）。
...
5. 第 2 层选择维度：选定某个指标库后，显示该库的维度列表。原型 mock：无论选择哪个指标库，下钻内容均复用同一棵 环境→主题→议题→指标 树（环境维度下含 环境政策与管理体系、能源 等主题）。
...
8. 勾选多选：用户勾选指标，可勾选多个（多选）。

9. 提交：用户提交所选指标，一次可提交多个（提交按钮的命名与位置以原型为准）；下钻到指标层、勾选提交即结束，暂不含时间维度选择（年份/报告期等，后续计划扩展）。

10. 结果一次性返回：AI 对话框**一次性**返回查询结果，以**一个表格**承载本次提交的全部指标结果，显示在会话区，表格按全局规则 1 带"⚠ AI生成/提取，请人工核实"标注；结果表格的列构成与样式**待确认**（原型 mock 列：指标代码、指标名称、数值、单位、报告期、数据来源），提交的某指标无查询结果时在表格中如何表现**待确认**（建议口径：保留该指标行，数值等列以"—"占位）。
...
④ 搜索结果勾选：用户在搜索结果中勾选目标指标，效果与逐层勾选相同，可多选、可一并提交。

权限过滤口径**待确认**：本功能可选的指标与返回的数据是否按用户权限过滤（建议口径：遵循全局规则 6 AI 权限可见性，仅当前用户权限范围内的指标数据可被选取与返回）；本版原型未实现权限过滤。

本小节待确认事项汇总：① 结果表格列构成与样式、某指标无查询结果时的表现（见步骤 10）；② 权限过滤口径（见上方说明）；③ 选择界面形态（见步骤 2）；④ 搜索无匹配结果时的空态文案（见子树搜索 ③）。

验收清单（对应需求理解 REQ-01 v2 功能点 FP-1～FP-5 与交互步骤，逐条可验证）：

- [ ] FP-1：管理员打开助手，输入区上方按钮区在【指标信息采集表】之后可见【数据选择】按钮，点击可进入数据选择界面
...
- [ ] FP-4：下钻到指标层、勾选提交即结束，不出现时间维度选择（后续计划扩展）
- [ ] FP-5：提交后 AI 一次性以一个表格返回本次全部提交指标的结果
- [ ] FP-5：结果表格带"⚠ AI生成/提取，请人工核实"标注（全局规则 1）
- [ ] 待确认项拍板后复核：结果表格列构成与无数据表现、权限过滤口径、选择界面形态、搜索空态文案
