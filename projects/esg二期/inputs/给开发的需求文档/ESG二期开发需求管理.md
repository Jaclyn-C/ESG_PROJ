...
### 6.2.6 数据选择（指标下钻、搜索、批量提交·年份与维度逐指标配置）【新增】

> 【新增内容内部说明】本小节为 2026-09-04 增量新增，同日经历 v2 → v3 → v4 三轮更新（现依据需求理解 REQ-01 v4：v2 建立指标下钻、搜索与批量提交框架，v3 追加年份选择与维度选择并作全局查询条件，v4 经用户纠正改为已选指标条目内**逐指标配置**年份与维度——"维度和时间都放在具体的指标里选，不是全局的"）；三轮更新均只限本小节内部扩展改写，不改动本章节既有小节（6.2.1～6.2.5）与全局规则原文。与既有条目的衔接：① 可见性——【数据选择】按钮仅对管理员可见，处理方式对照全局规则 7 中【报告生成】【指标信息采集表】仅管理员显示的先例（无对应模式与提示），该约定仅在本小节内生效，全局规则 7 原文维持不变；② AI 标注——AI 返回的查询结果表格属 AI 生成内容，直接按全局规则 1 执行（"⚠ AI生成/提取，请人工核实"），不另立标注规则；③ 助手既有功能（三种模式、📎 上传、🗂 知识库引用、历史对话管理）维持现状，不因本小节调整。

功能说明：管理员可通过【数据选择】按指标层级逐层下钻（或搜索）选定指标；**年份与维度不是全局查询条件，而是在已选指标区内逐指标配置**——每个已选指标条目各自携带年份（可选，不选默认当年，当前＝2026）与维度（可选，不选默认 FII），不同指标可互不相同，以「指标名 + 所处层级 + 该指标自己的年份 + 该指标自己的维度对象」确定具体数据；指标层级共 5 层——指标库 → 维度 → 主题 → 议题 → 指标；指标可多选，一次提交全部已选指标（每个指标携带各自年份与维度，不做任何配置＝全部按 当年＋FII），提交后 AI 对话框对本次提交的全部指标**一次性**返回查询结果，以**一个表格**承载（行＝Σ(每个指标 × 该指标所选维度对象数)）。
...
11. 条目内维度配置（可选）：点击条目内【维度选择】，弹出维度选择二级弹窗设定**该指标**的维度口径（见下方「维度选择二级弹窗」步骤块）；**确认结果只落到当前条目**；可不配置，不选默认 FII。

12. 条目移除：条目可移除；移除时该指标的年份/维度配置随之丢弃（不可恢复）。

13. 提交：用户一次提交全部已选指标，每个指标携带各自的年份与维度；不做任何配置直接提交＝全部指标按 当年＋FII 查询（提交按钮的命名与位置以原型为准）。

14. 结果一次性返回：AI 对话框**一次性**返回查询结果，以**一个表格**承载本次提交的全部指标结果，显示在会话区，表格按全局规则 1 带"⚠ AI生成/提取，请人工核实"标注；表格行＝Σ(每个指标 × 该指标所选维度对象数)（维度未选的指标按 1 个默认对象 FII 计；行构成已定稿，不再是待确认项），年份列、维度对象列逐行取该指标各自的配置，列集与列序**待确认**（默认口径列：指标代码、指标名称、年份、维度对象、数值、单位、数据来源），提交的某指标无查询结果时在表格中如何表现**待确认**（建议口径：保留该指标行，数值等列以"—"占位）。
...
维度选择二级弹窗（从已选指标条目内的【维度选择】入口打开，见主流程步骤 11；叠于数据选择弹窗之上）：

① 打开二级弹窗：用户在**某条已选指标条目内**点击【维度选择】，弹出维度选择二级弹窗（数据选择弹窗保持打开不关闭；风格对齐现有弹窗，如【🗂】知识库选择弹窗）。

② 四维度单选切换：二级弹窗提供四个维度选项（单选）：1、FII　2、分会层级　3、法人层级　4、策进组层级；切换维度后仅当前所选维度下的选项可勾选。

③ 选 FII：仅 1 个选项「FII」，勾选即**该指标**的查询口径为 FII 整体。
...
⑦ 维度可不选：该指标不设定维度时，默认 FII（结果中该指标按 1 个默认维度对象「FII」展开）。

⑧ 确认与回显：确认后关闭二级弹窗，**选择结果只落到当前配置的这个指标条目**——条目内回显维度名＋已选对象摘要（如「分会层级：CESBG、SEG 等 3 项」），可清除，清除后该指标回到默认 FII；**其他指标条目不受影响**。

维度选项规格与数据来源（各指标条目共用同一套维度选项；**选择结果只作用于当前配置的指标条目**）：

- **FII**：仅 1 个选项「FII」（无截图，用户口述）。
...
边界口径：① 同一次提交中，不同指标的年份/维度可互不相同，结果各行按各自配置返回；② 条目未配置的逐条目取默认值——年份未选＝当年（当前＝2026），维度未选/已清除＝FII（该指标按 1 个默认对象展开）；③ 移除指标条目即丢弃该条目的年份/维度配置（不可恢复）。

本小节待确认事项汇总：① 结果表格列集与列序、某指标无查询结果时的表现（见步骤 14；行构成已定稿＝Σ(每个指标 × 该指标所选维度对象数)，不再是待确认项）；② 维度二级弹窗是否需要提供搜索（见子树搜索 ⑤，当前默认＝不提供，搜索仅作用于指标选择）；③ 权限过滤口径（见上方说明）；④ 搜索无匹配结果时的空态表现（见子树搜索 ③）。

验收清单（对应需求理解 REQ-01 v4 功能点 FP-1～FP-7 与交互步骤，逐条可验证）：
...
- [ ] FP-4：提交口径（指标级）＝每个指标携带各自的年份与维度一次性提交，未做任何配置的指标按默认（当年＋FII）提交
- [ ] FP-4：移除已选指标条目后，该条目的年份/维度配置随之丢弃，不参与本次提交
- [ ] FP-5：提交后 AI 一次性以一个表格返回本次全部提交指标的结果
- [ ] FP-5：结果表格行＝Σ(每个指标 × 该指标所选维度对象数)（维度未选的指标按 1 个默认对象 FII 计）；年份列、维度对象列逐行取该指标各自的配置
- [ ] FP-5：结果表格带"⚠ AI生成/提取，请人工核实"标注（全局规则 1）
- [ ] FP-6：每个已选指标条目内各自提供年份选择（默认口径：年份下拉），可不选；不选时该指标按当年（当前＝2026）处理，年份按指标各自生效
- [ ] FP-7：每个已选指标条目内各自提供【维度选择】入口，点击弹出二级弹窗（叠在数据选择弹窗之上），四个维度（FII / 分会层级 / 法人层级 / 策进组层级）单选切换
- [ ] FP-7：FII 维度仅 1 个选项「FII」，勾选即该指标的查询口径为 FII 整体
- [ ] FP-7：分会层级为扁平列表（13 项），可多选
- [ ] FP-7：法人层级选项显示法人代码＋法人名称（10 项），可多选
- [ ] FP-7：策进组层级两级结构——只勾一级策进组＝覆盖该组整组，也可勾选二级组织，组内可多选
- [ ] FP-7：维度可不选或清除，未选/清除后该指标默认按 FII 处理
- [ ] FP-7：维度确认后回显在当前指标条目内（维度名＋已选对象摘要），可清除；其他指标条目不受影响
- [ ] FP-6/FP-7：同一次提交中两个指标配置不同年份/维度时各自生效，结果各行按各自配置返回
- [ ] 待确认项拍板后复核：结果表格列集与列序及无数据表现、维度弹窗是否需要搜索、权限过滤口径、搜索空态表现
