...
5. 首次使用无前置条件：全局映射库为空也可直接发起（不强制上传映射表），未命中映射的行全部由大模型语义匹配。
...
1. 年份选择按钮与判定：输入区按钮排在【数据选择】之后，**仅在"指标信息采集表"模式下显示**；点击弹出年份下拉，选项＝平台**全部收集年份**（按收集年份字段由新到旧排列，第一项为最新收集年份），**默认选中最新收集年份**（不提供"自动"）。年份一律经按钮显式选择（取消"消息中明示年份"判定）。所选年份与模板封面年度不一致时**不阻止**，提示"所选年份（XXXX）与模板封面年度（YYYY）不一致，请确认"后继续。上期年份＝本期减一年；年份整表统一；两列列头标注所选年份（如"本期指标数据（2025年）/上期指标数据（2024年）"）。年份选择对下一次提取生效，提取后变更年份须重新提取。

2. 年份与数据匹配：**上传模板的年份不做限制**（任意年份的采集表模板均可使用，系统不对模板封面年度做校验拦截）；提取时按用户所选年份查询收集数据——所选年份**存在已填报数据** → 正常取数；所选年份**无任何收集数据**（如该年份仅发起过收集但无人填报）→ 提示"所选年份（XXXX）暂无收集数据，请更换年份"，不生成确认表，用户更换年份后重新提取。
