...
> ⚠️【需求变更 2026-09-14】本节 6.3.1–6.3.3 为旧版流程，**已作废**（保留仅作历史记录，请勿参照开发）；当前有效版本为本节末尾【6.3.4–6.3.10 新版流程 · 2026-09-14】。
...
> 【6.3.4–6.3.10 新版流程 · 2026-09-14】变更摘要（相对测试环境现状）：
> ① 首条消息即建工作流 → 改为正常对话＋语义触发：每条文字消息做生成意图判定，具备"开始生成"类意图才建工作流（唯一发起方式，无兜底按钮、无二次确认弹窗，宁可触发）；
> ② 新增会话级"上下文（Context）"：历轮附件/知识库引用/数据选择登记项累积、可增删，建流时整体提供给模型（现状：无累积机制）；
> ③ 确认交互由 radio 候选行确认卡 → 改为与 6.4.7 采集表同款的确认表（预览卡＋完整表格大窗、匹配来源徽标、行尾状态列），与采集表两处差异＝大窗顶部"添加指标"行＋单列"数据"（取最新收集年份），界面文案纯文字（现状：单卡候选行，无表格无大窗）；
> ④ 新增数据选择联动：数据选择确认项登记进上下文，建流增传 data_selections[] 并直置已匹配行（现状：数据选择与报告零联动）；
> ⑤ 新增独立"报告映射表"：终确时"采纳且有平台编码"行入库，作为参考文件注入大模型提取/匹配（软参考）；与采集表全局映射库两表两套消费、互不涉及（现状：报告确认后不写任何映射）；
> ⑥ 知识库文件类型（参考文档/提示词文档）在弹窗显示并在报告链路消费：提示词文档进要求侧、参考文档进素材侧（现状：类型已透传但弹窗不显示、agent 零消费）；
> ⑦ 撰写口径定稿为生成式撰写：AI 按用户要求与确认指标撰写、数值取自平台已确认数据（原"原文逐字保留，仅将指标数值替换为接口最新值"口径废除）。

### 6.3.4 进入与正常对话

功能说明：报告模式即正常 AI 对话（与网页版 AI 体验一致），用户随时发消息、AI 凭自身能力流式回答；附件、知识库引用与数据选择登记项在会话内累积为"上下文（Context）"，供生成时整体使用。

1. 进入：点击【报告生成】切换模式，顶部显示模式提示条，助手发送欢迎语说明用法。

2. 正常对话：用户可随时发送任意内容——文字（要求、补充信息、提问，或用自然语言点名要写的指标名）、点击【📎】上传本地参考文件或提示词模板、点击【🗂】从知识库引用文件（弹窗内文件行显示类型：参考文档/提示词文档）；AI 凭自身能力正常流式回答，附件与引用随该轮消息参与回答。

3. 上下文（Context）累积：会话内历轮的附件、知识库引用与数据选择登记项累积为会话级上下文（可逐项增删）；正常对话轮发送后附件照常随轮发送，上下文保留。

4. 仅附件无文字也可发送（沿用 6.2.4）：发送后文件登记进上下文，AI 回复"已收到《××》，已登记为报告素材"；无文字即无语义判定，纯附件不触发工作流。

5. 数据选择联动（可选）：点击输入区【数据选择】按钮打开弹窗（现有功能，配置交互不变），确认后结果表格卡片照旧落会话展示，同时所选指标与数值登记进上下文。

### 6.3.5 语义触发

功能说明：报告模式的每条文字消息都做"开始生成报告"意图判定（模型语义判定，实现由开发定）；语义触发为唯一发起方式——无兜底按钮、无"是否开始生成"二次确认弹窗，意图判定倾向宁可触发（误判可取消工作流，漏判由用户换更明确的说法重述）。

1. 触发：用户消息具备生成意图（如"请帮我生成""开始生成"等）即由该条消息创建报告工作流，进入提取、确认、生成链路。

2. 建流载荷：上下文全部内容＋触发消息＋会话历史（聚合方式由开发定，单条消息不超过 5 万字符）。

3. 门禁沿用现状：会话内已有非终态工作流时拒绝新发起，期间发消息按正常对话回答；工作流终态或取消后放行，可再次语义触发新建工作流。

### 6.3.6 提取与匹配

功能说明：工作流创建后，LLM 从素材侧（参考文档与粘贴内容）提取定量指标（3–12 个），逐项与平台指标做语义匹配给出 top3 候选；独立的报告映射表作为参考文件注入提取/匹配 prompt（软参考——辅助模型识别文章表述与平台指标的对应，不做硬匹配拦截、不自动带出行），注入按相关性取子集控制规模；提示词文档文本进"要求/指令"侧，注入提取与生成 prompt。

1. 提取范围＝新素材提取出且不在会话级"已确认指标集"内的项（首轮即全部提取项；已确认项不重复出现、继续生效，判重键暂定按平台指标）。

2. 匹配＝大模型语义匹配 top3（现状）＋报告映射表软参考注入；采集表侧"上传映射表→全局库→语义匹配"硬优先级属 6.4.6 机制，报告链路不使用。

3. 数据选择登记的行直接置为已匹配行（显示所选数值），跳过提取/匹配；与 AI 提取撞同一平台指标时合并为一行、数据选择优先。

### 6.3.7 确认表（采集表同款＋两处差异）

功能说明：确认表与 6.4.7 采集表确认表同款（会话内预览卡＋完整表格大窗），列集与交互照采集表；与采集表两处差异——①大窗顶部"添加指标"行②数据列（采集表为本期/上期两列，报告为单列"数据"，取最新收集年份）；界面文案纯文字、无符号。

1. 预览卡：提取完成后助手回复"解析完成：提取到 × 个指标"并出现确认表预览卡；超过 6 行仅显示前 6 行＋提示行＋【查看/编辑完整表格】按钮。

2. 大窗：承载全部编辑——副本编辑、点确认才应用、关闭或取消丢弃修改。

3. 列集与交互（照采集表）：序号 / 素材指标名（只读）/ 平台对应指标名（top3 下拉带置信度，候选经 0.5 阈值过滤、默认最高分、都不对可手动输入）/ 平台对应指标编码（只读、随匹配联动）/ 数据（单列：取平台收集年份中最新的年份的已收集数据，平台带出、可编辑；数据选择行显示所选数值）/ 匹配来源（徽标：AI 提取、数据选择、手动添加，含双标签）/ 是否采纳 / 行尾状态列（已修改、已刷新、低置信，口径同采集表）。

4. 添加指标（与采集表差异之一）：在大窗顶部"添加指标"行输入指标名或参考文件/模板里的表述原文；添加时不匹配——新行仅有素材指标名＋匹配来源"手动添加"＋状态"待匹配"；点【确认匹配】后才与本次其他修改一起重匹配并返回二次确认（全量表），此时该行 top3 呈现与 AI 提取行一致，匹配来源为双标签"手动添加＋AI匹配"；平台未命中显示"手动添加＋未匹配"，可继续手动输入。

5. 轮内循环确认：表上存在任何修改（改选候选、切换跳过、手动输入、添加指标）→ 重匹配 → 返回全量确认表再确认，只要有修改就再次全量确认、循环往复（沿用 6.4.8 循环确认逻辑与现状代码行为）。

6. 终确＝落库点：点【确认匹配】且本次无任何修改 → 本轮终确，确认表锁定，同时发生三件事——①本轮映射写入报告映射表（见 6.3.9）②会话级已确认指标集更新③进入生成（基于累积全集）；全部行均不采纳 → 提示"至少采纳一项"、不进入生成。

7. 校验：手动选择了输入但未填指标名时点【确认匹配】，提示并定位到该输入框、不关闭。

### 6.3.8 生成与交付

功能说明：终确后 LLM 按用户要求生成式撰写报告草稿（要求侧＝建流前会话内全部用户消息，含提示词文档；事实侧＝累积的已确认指标集），交付可编辑纯文本；图表与排版由用户后期处理。

1. 生成：终确后助手显示"正在生成报告"，完成后回复"报告生成完成"。

2. 查看与编辑：出现"报告生成结果"卡片，标题旁带"AI生成/提取，请人工核实"角标；正文为可编辑纯文本（点击即可修改，失焦保存，并发乐观锁保护）。

3. 复制与下载：【复制全文】复制正文到剪贴板；【下载 .docx】下载 Word 草稿（文件含同样的 AI 生成标注）。

4. 累积式多轮：生成后继续补充（新参考文献/新要求）→ 正常对话回答 → 再次语义触发 → 新一轮确认（只含未确认过的新增项，表内修改仍走全量循环确认直至终确）→ 基于累积全集重新生成；每轮新结果卡追加落会话、旧卡保留；已生成草稿的修改沿用结果卡手动编辑（对话式迭代改写草稿本期不做）。

5. 长度限制：单次内容超模型处理范围提示"当前提问内容过长，请拆分后重试"；生成超上下文导致无法续写时会话中断、提示重新发起，已生成内容须用户自行复制保存。

### 6.3.9 映射沉淀（报告映射表）

功能说明：报告确认结果写入独立的报告映射表，与采集表全局映射库（6.4.9 双写机制）彻底分开——两表、两套消费，互不涉及；报告映射表为纯后台资产，本期无管理界面，不出现在 6.1.7"信息采集指标映射表"列表。

1. 入库时机与范围：每轮终确时，对"采纳且有平台编码"的行写入报告映射表；每轮终确各自落库一次，同一表述按 upsert 更新（不重复堆积）；不采纳行、无映射行不入库。

2. 条目结构：表述 → 平台指标名＋平台指标编码＋来源标签；存两类映射——文章语句→平台指标、用户自然语言→平台指标。

3. 护栏：①来源标签（文章提取/手动添加/自然语言，命名待确认）②入库表述长度上限暂定 100 字档（用途＝控制注入 token 规模），整段长句不入库。

4. 消费方式：作为参考文件注入大模型的提取/匹配 prompt（软参考，不是硬匹配）；模型参考后产出的候选仍走 top3＋确认表，误配可改；同一文章措辞漂移导致参考失效时由语义匹配兜底，确认后新措辞再入库，逐步收敛。

### 6.3.10 边界

1. 语义误触发：可取消工作流（沿用现有取消能力），或素材已足够时顺势继续确认；触发消息内容不丢失（已进建流上下文，上下文保留）。

2. 语义漏触发：模型按普通消息回答；用户换更明确的说法再次表达生成意图即可。

3. 非终态工作流进行期间发消息：按正常对话回答、不新建工作流；终态或取消后放行再次发起。

4. 累积文字超 5 万字符：沿用现有超长提示"当前提问内容过长，请拆分后重试"。

5. 未做任何数据选择就发起：数据选择为空，正常走提取/匹配。

6. 报告映射表条目：仅作模型参考注入，不硬匹配、不自动带出行。

7. 知识库文件类型：上传时类型必选沿用现状；AI 选择弹窗显示类型；知识库列表/卡片补类型列（是否纳入本期待确认）。
