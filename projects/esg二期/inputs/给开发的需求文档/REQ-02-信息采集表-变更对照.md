...
- **目标**：输入区新增【📅 年份】按钮，排在【数据选择】之后，**仅在"指标信息采集表"模式显示**（切换到其他模式隐藏）；点击弹出年份下拉，选项＝平台**全部收集年份**（按收集年份字段由新到旧排列，第一项为最新收集年份），**默认选中最新收集年份**（不提供"自动"）。年份一律经按钮显式选择（取消"消息中明示年份"判定）；所选年份与模板封面年度不一致时**不阻止**，提示"所选年份（XXXX）与模板封面年度（YYYY）不一致，请确认"后继续；年份对下一次提取生效，提取后变更须重新提取（6.4.7 第 1 条）。**上传模板的年份不做限制**（不对模板封面年度做校验拦截）；提取时按所选年份查询收集数据，所选年份**无任何收集数据**（如仅发起过收集但无人填报）→ 提示"所选年份（XXXX）暂无收集数据，请更换年份"，不生成确认表（6.4.7 第 2 条）。
- **测试环境现状（代码位置）**：前端 `csr-esg-web/src/components/ai-assistant/` 按钮区无年份按钮（chat-input.vue 模式按钮排仅报告/采集表/数据选择，且按 admin 过滤）；年份下拉的数据源需平台提供"全部收集年份"查询（按收集年份字段去重、由新到旧，建议同时返回各年份是否有已填报数据以支持下拉"暂无数据"标注与提取拦截）；后端 `esg-agent-service/src/services/collection-form-template.ts` 的 resolveCollectionReportingYear——判定链含"消息正则抓年份"，且请求年份与封面年不一致时直接抛错阻止（CollectionTemplateValidationError），需改为提示继续；CreateWorkflowInput 已支持显式 reportingYear 入参，前端补传即可。
- **改动类型**：新增（前端按钮＋年份列表数据源＋无数据拦截提示）＋修改（后端判定链去掉消息年份、不一致由阻止改为提示）。
- **验收标准**：①按钮仅在采集表模式可见、位于【数据选择】之后；②下拉列出平台全部收集年份、由新到旧、第一项标"最新"且默认选中；③选与封面不一致年份出现提示文案且流程不被阻断，提取后列头按所选年份显示（如选 2024 → "本期指标数据（2024年）/上期指标数据（2023年）"）；④选择对下一次提取生效；⑤选"发起过收集但无填报数据"的年份后提取 → 提示"暂无收集数据，请更换年份"、不出确认表，更换年份后可正常提取。
